...
class Warehouses(Resource):
    ...
    def post(self):
        data = get_data_from_request_or_raise_validation_error(
            BulkWarehouseValidator, request.json
        )
        created_warehouses, error_data = WarehouseController.add_warehouses(
            data["warehouses"]
        )
        return make_response(
            jsonify(warehouses=created_warehouses, error_data=error_data), 201
        )
...
class BenchmarkProductivity(Resource):
    def put(self):
        data = get_data_from_request_or_raise_validation_error(
            UpdateBenchmarkProductivityValidator, request.json
        )
...
class WarehouseDemands(Resource):
    def get(self, warehouse_id: int):
        ...
        demands = DemandController.get_demands_by_warehouse_id(
...
class Demands(Resource):
    def put(self):
        data = get_data_from_request_or_raise_validation_error(
            UpdateDemandValidator, request.json
        )
...
class CalculateManpower(Resource):
    def post(self):
        data = get_data_from_request_or_raise_validation_error(
            RequirementValidator, request.json
        )
...
class ProductivityFile(Resource):
    def post(self, warehouse_id: int):
        ...
        if not WarehouseController.get_warehouse_by_id(warehouse_id):
            return make_response(
                jsonify(
                    {"error": f"Warehouse not found with id {warehouse_id}"}
                ),
                403,
            )
        ...
        if not file.filename.endswith(".xls") and not file.filename.endswith(
            ".xlsx"
        ):
            return make_response(
                jsonify({"error": "Invalid file extension."}), 400
            )
        ...
        if missing_columns:
            return make_response(
                jsonify({"error": f"Missing columns: {missing_columns}."}), 400
            )
        elif extra_columns:
            return make_response(
                jsonify({"error": f"Extra columns: {extra_columns}."}), 400
            )
        ...
        (
            category_data,
            error,
        ) = CategoryController.convert_excel_file_data_according_to_category(
            data
        )
        if error:
        ...
        BenchmarkProductivityController.add_benchmark_category_from_excel_file_data(
            data, warehouse_id
        )
...
class DemandFile(Resource):
    def post(self, warehouse_id: int):
        ...
        if df.columns[0].lower() != "date":
            return make_response(
                jsonify({"error": "Date column is missing"}), 400
            )

        invalid_categories = CategoryController.check_invalid_categories(
            df.columns[1:]
        )
...
wmp_namespace.add_resource(
    ...
    "/benchmark_productivity/<int:warehouse_id>",
)
...
wmp_namespace.add_resource(
    ProductivityFile, "/upload_productivity_file/<int:warehouse_id>"
)
wmp_namespace.add_resource(
    DemandFile, "/demand_forecast_file/<int:warehouse_id>"
)
