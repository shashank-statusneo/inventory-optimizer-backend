...
class BulkWarehouseValidator(Schema):
    warehouses = fields.List(
        fields.Nested(WarehouseAndCategoryValidator()), required=True
    )
...
class BulkBenchmarkProductivityValidator(Schema):
    productivity = fields.List(
        fields.Nested(BenchmarkProductivityValidator()), required=True
    )


class UpdateOnlyFieldBenchmarkProductivity(Schema):
    id = fields.Float(required=True)
    productivity_experienced_employee = fields.Float(
        required=False, validate=Range(min=0)
    )
    productivity_new_employee = fields.Float(
        required=False, validate=Range(min=0)
    )
...
class UpdateBenchmarkProductivityValidator(Schema):
    productivity = fields.List(
        fields.Nested(UpdateOnlyFieldBenchmarkProductivity()), required=True
    )
...
class DemandValidator(Schema):
    ...
    category_id = fields.Integer(required=True)
    date = fields.Date(
        required=True, validate=greater_or_equal_to_current_date
    )
...
class UpdateDemandValidator(Schema):
    demands = fields.List(
        fields.Nested(UpdateOnlyFieldDemand()), required=True
    )
...
class RequirementValidator(Schema):
    ...
    day_working_hours = fields.Integer(
        required=True, validate=Range(min=1, max=24)
    )
