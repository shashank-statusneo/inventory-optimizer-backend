...
class WarehouseController:
    ...
    @classmethod
    def add_warehouses(cls, warehouses_data: list):
        """
...
class CategoryController:
    ...
    @classmethod
    def convert_excel_file_data_according_to_category(cls, data: list):
        """
        ...
        for record in data:
            try:
                if isinstance(
                    record["productivity_experienced_employee"], float
                ):
                    decimal_part = str(
                        record["productivity_experienced_employee"]
                    ).split(".")[1]
                    if decimal_part != "0":
                        raise
                ...
                if isinstance(record["productivity_new_employee"], float):
                    decimal_part = str(
                        record["productivity_new_employee"]
                    ).split(".")[1]
                    if decimal_part != "0":
                        raise
                ...
                error_data.append(
                    f"Invalid value(s) for : {record['category']}"
                )
    ...
    def check_invalid_categories(categories: list) -> list:
        ...
        return [
            ...
            if category_name not in Category.category_name_to_id_mapping()
        ]


class BenchmarkProductivityController:
    @classmethod
    def add_benchmark_productivity(cls, benchmark_productivity: list):
        """
        ...
        for productivity in benchmark_productivity:
            try:
                ...
                _productivity = BenchmarkProductivity.query.filter_by(
                    category_id=productivity["category_id"],
                    warehouse_id=int(productivity["warehouse_id"]),
                ).first()
    ...
    @classmethod
    def get_benchmark_productivity_by_warehouse_id(
        cls, warehouse_id: int
    ) -> list[dict] or None:
        """
        ...
        :return:
        """
        records = BenchmarkProductivity.query.filter_by(
            warehouse_id=warehouse_id
        )
        return [record.serialize() for record in records]

    @classmethod
    def add_benchmark_category_from_excel_file_data(
        cls, file_data: list, warehouse_id: int
    ):
        """
        ...
        benchmark_productivity_data = [
            {
                ...
                "warehouse_id": warehouse_id,
                "productivity_experienced_employee": record[
                    "productivity_experienced_employee"
                ],
                "productivity_new_employee": record[
...
class DemandController:
    ...
    @classmethod
    def get_demands_by_warehouse_id(
        cls, warehouse_id: int, start_date: type, end_date: type
    ):
        """
    ...
    def create_input_demands_data(records) -> dict:
        ...
        for record in records:
            ...
            output[str_date]["total"] = (
                output[str_date].get("total", 0) + record.InputDemand.demand
            )
            output["total"][category_name] = (
                output["total"].get(category_name, 0)
                + record.InputDemand.demand
            )
            output["total"]["total"] = (
                output["total"].get("total", 0) + record.InputDemand.demand
            )
    ...
    def check_and_convert_excel_data_according_to_input_demand(
        ...
        for record in data:
            try:
                ...
                error_data.append(
                    f"Invalid format for date : {record['date']}"
                )
                continue
            ...
            for key in record:
                ...
                try:
                    ...
                    error_data.append(
                        f"Invalid Demand ({record[key]}) for date : {date} category : {key}"
                    )
                    continue
                if demand <= 0:
                    error_data.append(
        ...
        if valid_dates:
            error_data.append(
                f"Data not found for : [{', '.join(str(date) for date in valid_dates)}]"
            )
...
class ResultController:
    ...
    def calculate_manpower(cls, requirement_data: dict) -> dict:
        ...
        :return:
        """
        new_requirement = RequirementController.add_requirement(
            requirement_data
        )
        ...
        additional_data = {
            ...
            "total_hiring_budget": requirement_data["total_hiring_budget"]
            - randint(100, 1000),
        }
        return {
            ...
            "output": result,
            "demand_vs_fulfillment_data": cls.get_demand_vs_fulfillment_dummy_data(
                requirement_data
            ),
            "additional_data": additional_data,
            "warehouse_name": WarehouseController.get_warehouse_by_id(
                new_requirement.warehouse_id
            ).name,
        }
    ...
    def get_demand_vs_fulfillment_dummy_data(requirement_data: dict) -> dict:
        ...
        for date in requirement_data["expected_demand"]:
            ...
            for category in requirement_data["expected_demand"][date]:
                if category == "total":
                    dummy_output[date][category] = {
                        "expected_demand": requirement_data["expected_demand"][
                            date
                        ][category],
                        "fulfillment_with_current": int(
                            requirement_data["expected_demand"][date][category]
                            * random()
                        ),
                        "fulfillment_with_total": requirement_data[
                            "expected_demand"
                        ][date][category]
                        + randint(-250, 250),
                    }
                    ...
                    dummy_output[date][category] = {
                        ...
                        "fulfillment_with_current": int(
                            requirement_data["expected_demand"][date][
                                category
                            ]["demand"]
                            * random()
                        ),
